...
/* --- Custom Variant Button Logic --- */
function initCustomVariantButtons() {
  document.querySelectorAll('.custom-button-group').forEach(group => {
    const selectId = group.dataset.controlsSelect;
    const selectElement = document.getElementById(selectId);
    if (!selectElement) return;

    const labels = group.querySelectorAll('.custom-button-label');

    labels.forEach(label => {
      label.addEventListener('click', () => {
        const valueToSelect = label.dataset.value;

        // 1. Update the hidden select's value
        selectElement.value = valueToSelect;

        // 2. Dispatch a 'change' event so Dawn's variant-selects component picks it up
        selectElement.dispatchEvent(new Event('change', { bubbles: true }));

        // 3. Update the active class for styling
...
initCustomVariantButtons();

// Re-run the script if the product form is re-rendered (e.g., in quick-add)
document.addEventListener('shopify:section:load', initCustomVariantButtons);